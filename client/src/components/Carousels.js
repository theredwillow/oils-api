...
class Carousel extends Component {
  ...
  render() {
    ...
    if (this.props.levelLabel === "base") {
      ...
      infoBlurb =
        "Middle oils are the heart of your blend and will emerge after your top oil has faded. These oils will help balance your blend.";
    ...
    return (
      <div id={this.props.levelLabel}>
        {this.props.toShow === true && (
          <div className="info-label">{infoLabel}</div>
        )}
        {this.props.toShow === true && (
          <div className="info-blurb-level">{infoBlurb}</div>
        )}
          ...
          <section style={this.state.slider.cardStyle}>
            {this.props.currentLevel.map((card, i) =>
              this.props.history.location.pathname === "/user" ? (
                <Card2
                  updateBlends={this.props.updateBlends}
                  isLoggedIn={this.props.isLoggedIn}
                  history={this.props.history}
                  mood={this.props.mood}
                  content={card}
                  checked={this.state.currentOil === card.name}
                  toggleOil={this.toggleOil.bind(this)}
                  key={i}
                  level={this.props.levelLabel}
                />
              ) : (
                <Card
                  content={card}
